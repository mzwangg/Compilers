...
//             if (skip(inst))
//                 continue;
//             auto preInstIt = std::find(exprs.begin(), exprs.end(),
//             Expr(inst)); if (preInstIt != exprs.end())
//             {
//                 // TODO:
//                 把对当前指令的def的use改成对于preInst的def的use，并删除当前指令。
...
//             Expr expr(inst);
//             // 对于表达式a +
//             b，我们只需要全局记录一次，重复出现的话，用同一个id即可 auto it =
//             find(exprVec.begin(), exprVec.end(), expr); int ind = it -
//             exprVec.begin(); if (it == exprVec.end())
//             {
...
//         {
//             in[turn].clear();
//             std::set_intersection(outBB[*it].begin(), outBB[*it].end(),
//             in[turn ^ 1].begin(), in[turn ^ 1].end(), inserter(in[turn],
//             in[turn].begin())); turn ^= 1;
//         }
//         inBB[block] = in[turn ^ 1];
//         // 计算outBB[block] = (inBB[block] - killBB[block]) U genBB[block];
//         std::set<int> midDif;
//         std::set<int> out;
//         std::set_difference(inBB[block].begin(), inBB[block].end(),
//         killBB[block].begin(), killBB[block].end(), inserter(midDif,
//         midDif.begin())); std::set_union(genBB[block].begin(),
//         genBB[block].end(), midDif.begin(), midDif.end(), inserter(out,
//         out.begin())); if (out != outBB[block])
//         {
//             outBB[block] = out;
//             for (auto succ = block->succ_begin(); succ != block->succ_end();
//             succ++)
